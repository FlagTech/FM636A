...
class Data_reader():
    def __init__(self, path_name, mode='categorical', label_name=None):
        ...
        if mode in ('regression', 'binary', 'categorical'):
            if label_name is None:
                self.label_name = [
                    f[:(f.rfind('.'))] for f in os.listdir(path_name) if not f.startswith('.')]
                if len(self.label_name) > 2 and mode == 'binary':
                    raise ValueError(
                        'Found more than two files, please assign parameter `label_name`.')
            else:
    ...
    def read(self, maxlen=None,
        ...
        if self.mode == 'regression':
            file = open(self.path_name, encoding=encoding, errors='ignore')
            for line in file:
                try:
                    data = line.split(' ')
            ...
            file_list = [
                f for f in os.listdir(self.path_name) if not f.startswith('.')]

            for name in file_list:
                file = open(
                    self.path_name + os.sep + name,
        ...
        if maxlen is None:
            ...
            train_data = pad_sequences(
                train_data, maxlen=maxlen, truncating='post').astype('float')
    ...
    if arch_class_name == 'Sequential':
        ...
        for i, o_layer in enumerate(o_layers):
            ...
            if o_layer_class_name == 'InputLayer':
                ...
                weights = [0, 0]
                layer_weights = model.layers[i - has_input_layer].get_weights()
        ...
        raise ValueError(f'Unable to save non-sequential model: {arch_class_name}')